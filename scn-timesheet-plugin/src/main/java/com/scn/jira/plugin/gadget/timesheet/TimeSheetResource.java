...
{
	private static final org.apache.log4j.Logger log = org.apache.log4j.Logger.getLogger(TimeSheetResource.class);
	...
	public Response getTimesheet(@Context HttpServletRequest request, @QueryParam("targetUser") String targetUserName)
	{
		...
		{
			return Response
					.ok(new TimeSheetRepresentation(vm.getBody("templates/scn/timesheetportlet/", "timesheet-portlet.vm",
							getVelocityParams(request, numOfWeeks, reportingDay, targetUser.getDirectoryUser())))).cacheControl(getNoCacheControl())
					.build();
		}
	...
	private Map<String, Object> getVelocityParams(int numOfWeeks, int reportingDay, User targetUser)
	{
		...
		try
		{
			...
			TimeSheet ts = new TimeSheet(this.outlookDateManager, this.permissionManager, this.issueManager,
					this.searchProvider, this.visibilityValidator, this.userManager, this.searchRequestManager,
					this.groupManager, this.projectRoleManager, this.scnPermissionManager, this.fieldVisibilityManager);
			
			ts.getTimeSpents(user, startDate.getTime(), endDate.getTime(), targetUser.getName(), false, null, null, null, null,
					null, null, null, null);
