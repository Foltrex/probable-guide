...
public class IssueActionFilter extends BaseFilter {
    ...
    private Map<String, ContentItem> parseMultipartContent(HttpServletRequest request) throws IOException {
        Map<String, ContentItem> contentParams = new LinkedHashMap<>();

        MultipartStream stream = new MultipartStream(request.getInputStream(), getBoundary(request).getBytes(), DEFAULT_BUFSIZE, null);
