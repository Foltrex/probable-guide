...
public class ScnAggregateOriginalEstimateCFType extends AbstractScnTimeCFType {

    public ScnAggregateOriginalEstimateCFType(CustomFieldValuePersister customFieldValuePersister, GenericConfigManager genericConfigManager, TextFieldCharacterLengthValidator textFieldCharacterLengthValidator, JiraAuthenticationContext jiraAuthenticationContext, IScnExtendedIssueStore store, JiraDurationUtils durationUtils, IssueManager issueManager, IGlobalSettingsManager settingsManager) {
    ...
    protected Long getTime(IScnExtendedIssue extIssue) {
        ...
        return extIssue.getIssue().getSubTaskObjects().stream()
            ...
            .map(IScnExtendedIssue::getOriginalEstimate)
            .reduce(extIssue.getOriginalEstimate(), AggregateTimeTrackingBean::addAndPreserveNull);
    }
}
