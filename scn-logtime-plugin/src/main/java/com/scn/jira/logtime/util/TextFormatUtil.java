...
public class TextFormatUtil {
	...
	public static String timeToString2(String date) {
		...
		if (restSeconds >= SECONDS_IN_DAYS) {
			...
			if (days > 0) {
				dateString = dateString.length()==0?String.valueOf(days) + "d":dateString+ " " +String.valueOf(days) + "d";
			}
	...
	public static Long string2ToTime(String timeString) {
		...
		java.util.regex.Pattern p1 = java.util.regex.Pattern.compile("^\\s*(([0-9]*[0-9])w)*\\s*(([0-9]*[0-9])d)*\\s*(([0-9]*[0-9])h)?\\s*(([0-5]*[0-9])m)?\\s*$");
		...
		if (m1.find()) {
			...
			String h = m1.group(6);
			String m = m1.group(8);
			timeLong = (w != null ? Long.parseLong(w) : 0) * SECONDS_IN_WEEK + (d != null ? Long.parseLong(d) : 0) * SECONDS_IN_DAYS + (h != null ? Long.parseLong(h) : 0)
					* Duration.HOUR.getSeconds() + (m != null ? Long.parseLong(m) : 0) * Duration.MINUTE.getSeconds();
		}
	...
	public static Long stringToTime(String timeString) {
		...
		if (timeString != null && timeString.contains(":")) {
			...
			timeLong = Long.parseLong(parts[0]) * Duration.HOUR.getSeconds() + Long.parseLong(parts[1]) * Duration.MINUTE.getSeconds();
		}
	...
	public static String replaceHTMLSymbols(String stringToReplace) {
		return stringToReplace.replaceAll("<", "").replaceAll(">","").replaceAll("#","").replaceAll("\'", "").replaceAll("\"", "").replaceAll("$","").replaceAll(";", "").replaceAll("\n"," ").replaceAll("\r"," ");	
