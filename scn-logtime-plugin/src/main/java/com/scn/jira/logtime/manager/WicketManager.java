...
public class WicketManager {
	...
	public WicketRepresentation gerUserWicketTimeForthePeriod(String login,
		...
		List<String> datesForList = DateUtils.getDatesList(startDate, endDate,
				DateUtils.formatStringDay);
		...
		try {
			...
			wicketRepresentation = new WicketRepresentation(login, times,
					TextFormatUtil.timeToString(total));

		}
		return wicketRepresentation;
	}

	public Map<String, WicketRepresentation> gerUserWicketTimeForthePeriods(
			List<String> logins, Date startDate, Date endDate) {

		Map<String, WicketRepresentation> wicketRepMap = new HashMap<String, WicketRepresentation>();

		for (String login : logins) {
			login = login.trim();
			WicketRepresentation wick = gerUserWicketTimeForthePeriod(login,
					startDate, endDate);
	...
	public boolean gerUserWicketPermission(String login) {
		...
		try {
			...
			String sql = "SELECT TOP 1000 [login] FROM [Jira_DWH].[dbo].[wicket_managers]" 
					+ " where login ='"
					+ login+ "'";
			...
			while (rs.next()) {
				...
				System.out.println("WICKET MANAGER FOUND!!!! " + person);
				result = true;
			}
	...
	public  Map<String, Map<String, Integer>> gerUsersCalendar(
			List<String> logins, Date startDate, Date endDate) {
		...
		for (String login : logins) {
			login = login.trim();
			Map<String, Integer> userCalMap = gerUserCalendar(login,
					startDate, endDate);
	...
	public  Map<String, Integer> gerUserCalendar(String login,
			Date startDate, Date endDate) {
