...
public class ExtWorklogLogtimeStore implements IExtWorklogLogtimeStore {
	...
	public List<ExtWorklog> getExtWorklogsByProjectBetweenDates(boolean assignedCh, Project project, Date startDate,
			Date endDate, String user) throws DataAccessException {
		...
		for (GenericValue worklogGV : worklogExtGVs) {
			if (worklogGV != null) {
				...
				if (assignedCh && issue != null && issue.getAssignee() != null
						&& user.equals(issue.getAssignee().getName()) || !assignedCh) {
	...
	public Worklog createExtWorklog(Long issueId, String _worklogType, Long _timeSpent, String _comment,
			String authorKey, Date date) throws DataAccessException {
	...
	public void updateExtWorklog(Long _worklogId, String _worklogType, Long _timeSpent, String _comment)
			throws DataAccessException {
		GenericValue worklogGV = ComponentAccessor.getOfBizDelegator().findByPrimaryKey("Worklog",
				ImmutableMap.of("id", _worklogId));
		if (worklogGV == null)
	...
	public Worklog formWorklog(Long issueId, Long _timeSpent, Long id, String _comment, String authorKey, Date date) {
		Issue issue = this.issueManager.getIssueObject(issueId);
		Worklog wl = new WorklogImpl(this.worklogManager, issue, id, authorKey, _comment, date, null, null, _timeSpent,
				authorKey, date, date);
		return wl;
	}

	public Worklog convertToWorklog(WorklogManager worklogManager, Long wlId, Long issueId) {
		GenericValue gExw = ComponentAccessor.getOfBizDelegator().findByPrimaryKey("Worklog",
				ImmutableMap.of("id", wlId));
		if (gExw == null)
			return null;
		Issue issue = this.issueManager.getIssueObject(issueId);
		Worklog wl = new WorklogImpl(this.worklogManager, issue, wlId, gExw.getString("author"), gExw.getString("body"),
				gExw.getTimestamp("startdate"), gExw.getString("grouplevel"), gExw.getLong("rolelevel"),
				gExw.getLong("timeworked"), gExw.getString("author"), gExw.getTimestamp("startdate"),
				gExw.getTimestamp("startdate"));
	...
	public ExtWorklog convertToExtWorklog(WorklogManager worklogManager, GenericValue gv, Issue issue) {
		...
		ExtWorklog worklogExt = new ExtWorklog(worklogManager, issue, gv.getString("author"), gv.getString("body"),
